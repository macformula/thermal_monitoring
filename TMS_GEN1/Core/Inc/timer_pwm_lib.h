...
static void MX_TIM4_Init(uint32_t prescaler, uint8_t channel_1, uint8_t channel_2, uint8_t channel_3, uint8_t channel_4);
...
uint16_t timer_4_repetition_counter = 0;
...
uint16_t TIMER_4_PERIOD_MULTIPLIER; //(Timer period) x (TIMER_4_PERIOD_MULTIPLIER) = period for timer 4 ISR
...
static void Timer_Init_Base(uint8_t timer, uint16_t period, uint8_t do_trigger_ISR, uint8_t channel_1, uint8_t channel_2, uint8_t channel_3, uint8_t channel_4, uint16_t ISR_period)
{
	...
	if ((timer == 1 || timer == 2 || timer == 4) && (period > 0 && period < 65536) && (do_trigger_ISR == 255 || do_trigger_ISR == 1 || do_trigger_ISR == 0) && (channel_1 == 255 || channel_1 == 1 || channel_1 == 0) && (channel_2 == 255 || channel_2 == 1 || channel_2 == 0) && (channel_3 == 255 || channel_3 == 1 || channel_3 == 0) && (channel_4 == 255 || channel_4 == 1 || channel_4 == 0) && ISR_period % period == 0)
	{
		...
		switch (timer)
		{
			...
			case(4):
				...
				MX_TIM4_Init(prescaler,channel_1,channel_2,channel_3,channel_4);
				if(do_trigger_ISR==1)
				{
...
static void Calculate_Timer_Period_Multiplier(uint8_t timer,uint16_t period, uint16_t ISR_period)
{
	switch (timer)
	{
		...
		case(2):
			...
			break;
		case(4):
			TIMER_4_PERIOD_MULTIPLIER = ISR_period/period;
			break;
	}
}
...
static void PWM_Init(uint8_t timer, uint8_t channel, uint8_t duty_cycle)
{
	if ((timer == 1 || timer == 2 || timer == 4) && channel >= 1 && channel <= 4 && duty_cycle >= 0 && duty_cycle <= 100)
	{
